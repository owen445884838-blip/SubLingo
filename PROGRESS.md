...
- All four permanent signing environment variables remain absent. Alpha 4 must therefore be a validation/debug-signed GitHub prerelease; the unsigned AAB is not distributed, and this build is not suitable for application-store submission or production updates. Implementation branch: `codex/fix-release-download-engine-init`; PR: [#8](https://github.com/owen445884838-blip/SubLingo/pull/8).
