...
> Status precedence: the 2026-08-16 Alpha 4 release, the 2026-08-16 YouTube extractor update, the 2026-08-15 Release download-engine initialization fix, the 2026-08-01 API 36/16 KB Alpha 3 release, the 2026-08-01 Alpha 2 release, the 2026-07-29 vocabulary foreign-key fix, the 2026-07-26 review lemma/card-switch and bilingual-mapping fixes, the 2026-07-25 player/transcript and review-vocabulary fixes, the 2026-07-23 M5 acceptance and product update, the 2026-07-22 acceptance section, and the 2026-07-21 snapshot below are authoritative when older historical sections describe superseded behavior.

## Alpha 4 release (2026-08-16)

- Prepared `0.1.0-alpha.4` (`versionCode 4`) after the user completed PME110 acceptance of the real YouTube download fix. The release contains the Release-runtime reflection fix, bounded network/idle timeouts, the verified yt-dlp nightly update, `visionos` anonymous fallback, and sanitized challenge errors. Room remains at schema 13 with no migration or persisted-data format change.
- The clean Android Studio JBR 17 gate passed: `:app:testDebugUnitTest :app:lintRelease :app:assembleRelease :app:verifyReleasePageSize :app:bundleRelease :app:generateReleaseSbom`. All 139 unit tests passed; Release Lint generated no error-severity findings; R8/resource shrinking, APK/AAB packaging, the 151-component SBOM, and 12 APK plus 282 nested Python ELF 16 KB checks succeeded. Lint 8.13.2 printed a non-fatal missing transient cache-jar diagnostic during analysis, but both the clean gate and a standalone rerun completed successfully and produced the reports.
- Exact Alpha 4 validation APK: 226,776,857 bytes, SHA-256 `734a1f3894041712970717d259028779c90c7f5ad7cabec09b3b21dd45dbdba4`; `aapt2` confirms version code/name 4 / `0.1.0-alpha.4`, `apksigner` verifies v2/v3 with the Android debug certificate, and `zipalign -P 16` passes. Unsigned AAB: 231,274,383 bytes, SHA-256 `178cff8ce30e132b532fab3bcbf6b9d29c2ec4a9e9689dfac6b93638294d2252`. SBOM SHA-256: `9b2c9c06c0aa3bb8dcc448727337450d26c40165c5382c4efae74ed64c448053`. R8 mapping SHA-256: `d986bcd7a0900bbd469b025942101485ac71a8d4f06755b7e5a1eee361c97291`.
- The exact signed APK upgraded PME110 (`3B1686008AK00000`, API 36) in place from Alpha 3 to Alpha 4. `firstInstallTime` remained `2026-08-16 12:00:41`, the previously downloaded 119,262,971-byte YouTube MP4 plus metadata and thumbnail remained readable, the existing review session rendered, and the app stayed resumed with an empty crash buffer and no app-specific fatal, Room/SQLite, R8, or native-link failure.
- Bundletool 1.18.3 calculated a split-delivery range of 57,996,909 to 68,246,104 bytes from the AAB and produced a 226,860,331-byte universal APK (SHA-256 `c5d166313a627309fefb28d43fa57dce7193ae21fe5e180df2597bb9871481fd`). USB disconnected after the exact APK device validation and before bundletool could capture a fresh PME110 device spec, so the release records the verified range rather than claiming one exact PME110 split size.
- All four permanent signing environment variables remain absent. Alpha 4 was published as a validation/debug-signed GitHub prerelease; the unsigned AAB was not distributed, and this build is not suitable for application-store submission or production updates. Implementation [PR #8](https://github.com/owen445884838-blip/SubLingo/pull/8) merged as commit `d1833ba`; annotated tag `v0.1.0-alpha.4` and the [Alpha 4 prerelease](https://github.com/owen445884838-blip/SubLingo/releases/tag/v0.1.0-alpha.4) point to that merge. GitHub reports all three assets uploaded, and every server-side digest matches the local SHA-256 recorded above.

## YouTube extractor and anonymous-client update (2026-08-16)

- PME110 Logcat and the on-screen failure identified a stale extractor failure, not a network timeout or media-mux failure: youtubedl-android 0.18.1 embeds yt-dlp `2025.11.12`, which failed the current YouTube `n` challenge with QuickJS (`found 0 n function possibilities`), exposed only image formats, and ended with `Requested format is not available`. The installed app was the public `0.1.0-alpha.3`, so it did not contain this branch's later runtime fixes.
- youtubedl-android 0.18.1 remains its latest Maven/GitHub release and its repository has not advanced since 2025-11-16. yt-dlp stable is `2026.07.04`; current upstream reports confirm YouTube regressions in the old `android_vr` route and recommend the nightly channel while fixes are moving. SubLingo now packages the official yt-dlp nightly `2026.08.16.020253` zipapp (SHA-256 `33075a8f5a0ee7189302dde76d0caab8a2c1c4a1b82235ed761814df89e0900d`) and uses the current anonymous `visionos` client fallback.
- Runtime initialization verifies the embedded zipapp hash and atomically replaces any stale extracted yt-dlp copy after an app upgrade. A private preference records the installed engine version. This does not change Room schema or user media/job/subtitle/vocabulary/review data; the runtime copy lives under the app's no-backup files directory.
- YouTube JavaScript-challenge / only-images diagnostics now become a concise engine-update message rather than exposing raw yt-dlp output or incorrectly routing the user to login. The CycloneDX Release SBOM explicitly inventories the vendored nightly in addition to the youtubedl-android Maven wrapper, and third-party notices identify its upstream and license.
- Automated validation with Android Studio JBR 17: clean `:app:testDebugUnitTest :app:lintRelease :app:assembleRelease :app:verifyReleasePageSize :app:bundleRelease :app:generateReleaseSbom` passed (139 tests, 0 failures/errors); all 12 APK ELF files and 282 Python-archive ELF files passed the 16 KB check, and the SBOM contains 151 components. The optimized APK retained the official yt-dlp bytes and hash. `connectedDebugAndroidTest` could not start tests because ColorOS made the test-APK install command unresponsive; this was an installation/infrastructure failure, not an assertion failure.
- PME110 (`3B1686008AK00000`, API 36, 4 KB pages) then completed a real anonymous YouTube download for `https://www.youtube.com/watch?v=gUFP3dx01wc`: title `rawdogging photos in nyc`, `media.mp4` 119,262,971 bytes, metadata 945,270 bytes, and thumbnail 196,312 bytes. Download WorkManager finished successfully, audio segments were generated, and the job advanced to the expected `等待配置` subtitle state because the fresh install had no STT provider configured. No QuickJS, `n`-challenge, only-images, or requested-format error recurred. The exact MP4 pulled from the device passed local `ffprobe`: 1920x1080 H.264 High video, AAC-LC 44.1 kHz stereo audio, 916.027 seconds, SHA-256 `66b7a9fba5732babdba0d4090485d54e6e88f15f2bb47c3f33d0d85cb2aa1f80`.
- Validation-only Release APK before the Alpha 4 version bump: SHA-256 `b00a7cd6f0ec496a418e6de5de5704da6d0de1008cfe41c5a1dc723e7d3fabae`; v2/v3 signing verified. UTP's uninstall-after-test behavior removed the previously installed Alpha 3 before this validation build was installed, so that specific download run was a fresh install and did not test in-place data retention. The exact Alpha 4 upgrade validation is recorded above.
...
- Automated validation with Android Studio JBR 17: `:app:testDebugUnitTest :app:lintRelease :app:assembleRelease :app:verifyReleasePageSize` passed (137 unit tests, 0 failures/errors). The Release mapping confirms retained public constructors and ZIP interface methods for the reflected implementations; `git diff --check` passed.
- OPPO PME110 (`3B1686008AK00000`, API 36, 4 KB pages) validation passed after the fix. The unsigned Release APK was signed with the same local validation/debug certificate as the installed Alpha 3 and upgraded in place; `firstInstallTime`, app UID/data directory, and data inode were retained. Cold start logged `yt-dlp, FFmpeg and aria2c initialized (pageSize=4096)` with no `ef.e`, fatal exception, or ANR. A WorkManager download using the non-media URL `https://example.com` passed runtime initialization and executed both yt-dlp attempts before returning the expected `Unsupported URL` extractor failure, proving the optimized worker path without downloading media or starting cloud processing. The test URL and pre-existing failed no-media task cards were cancelled afterward; no completed media was present or removed. Validation APK SHA-256: `57df0e7bb0dfc748d88905b885845ae9816b8b9593073a25de2eb18572e84e97`.
- Branch: `codex/fix-release-download-engine-init`.

## API 36 and 16 KB Alpha 3 release (2026-08-01)

- Upgraded the supported Android toolchain from AGP 8.6.1/Gradle 9.3 to the stable API-36-capable AGP 8.13.2/Gradle 8.13 pair. The application now compiles and targets Android 16 / API 36, with `versionCode 3` and `versionName 0.1.0-alpha.3`; minimum API 26 and Room schema 13 remain unchanged.
- Added `:app:verifyReleasePageSize` to the Release gate. It parses the actual APK rather than source dependencies and verifies 16 KB `PT_LOAD` alignment for all 12 packaged arm64-v8a/x86_64 ELF libraries plus 282 ELF libraries inside the required Python runtime archives. The task rejects unexpected non-ELF native entries and future 4 KB regressions in the active 16 KB runtime path.
- Known 16 KB limitation: the bundled FFmpeg archive contains five WebP libraries that remain 4 KB aligned. The existing runtime protection therefore continues to disable packaged FFmpeg and aria2 on page sizes above 4 KB and selects the yt-dlp single-file/native-mux plus Android `MediaExtractor` compatibility path. Python/yt-dlp is fully exercised; this release does not claim full FFmpeg functionality on 16 KB devices.
- Added the first connected device regression test. It initializes `YoutubeDlRuntime` and executes offline `yt-dlp --version`, validating the nested Python executable without a media request. The test passed on the official API 37.1 ARM64 16 KB AVD (`PAGE_SIZE=16384`) and the API 26 ARM64 AVD; the exact R8 Release APK also installed and cold-launched on both. API 26 logged only Compose's caught optional `Layout.TextInclusionStrategy` compatibility probe.
- The clean Release gate passed with Android Studio JBR 17: `./gradlew clean :app:testDebugUnitTest :app:lintRelease :app:assembleRelease :app:verifyReleasePageSize :app:bundleRelease :app:generateReleaseSbom`. All 134 unit tests passed; Release Lint, R8/resource shrinking, APK/AAB packaging, 16 KB verification, and the 150-component SBOM succeeded. Existing deprecation/opt-in and native debug-symbol stripping warnings remain non-blocking.
- OPPO PHY110 (`9b43a22c`, API 36) upgraded in place to the exact Alpha 3 validation APK with all seven persisted videos retained. Home rendered normally and app-specific startup logs contained no fatal, ANR, Room, SQLite, R8, Hilt, WorkManager, or native-link failure. Bundletool 1.18.3 calculated a 65,275,200-byte device delivery from the AAB.
- Validation-only APK: 226,866,969 bytes, SHA-256 `50367e64dcb645488c364146fc93eadc472de0040ee56e00e29718ce4ec305fc`; `apksigner` verifies v2/v3 and `zipalign -P 16` passes. Unsigned AAB: 231,343,374 bytes, SHA-256 `74b8edbdbbd4b54981aec0b3c3860b2e165ea43d7f4f9f2e00f7e658d9934ae0`. SBOM SHA-256: `bda8fdfc22df001d3400191203993b167a6825de9fe0407344085e8b1ede5deb`. R8 mapping SHA-256: `92c1e63ada9a38336a412a38102fee10d674c6214f7dd82d65e086fdf42e9d34`.
- All permanent signing variables remain absent by explicit user decision. Alpha 3 was published as a validation/debug-signed GitHub prerelease and is not suitable for app-store submission. Implementation branch `chore/api36-16kb-alpha3` was merged by [PR #6](https://github.com/owen445884838-blip/SubLingo/pull/6) at commit `1a178a0`; annotated tag `v0.1.0-alpha.3` and the [Alpha 3 prerelease](https://github.com/owen445884838-blip/SubLingo/releases/tag/v0.1.0-alpha.3) point to that merge. GitHub reports all three assets uploaded and the server-side APK digest matches the local SHA-256 above.
...
- All four `SUBLINGO_RELEASE_*` variables were absent, so Alpha 2 remains an explicitly labeled validation/debug-signed GitHub prerelease. It is not a production/store artifact. The cumulative branch `fix/vocabulary-foreign-key` merged through Pull Request https://github.com/owen445884838-blip/SubLingo/pull/4 as commit `8447e7d`; tag `v0.1.0-alpha.2` and its APK/SBOM/R8 mapping are published at https://github.com/owen445884838-blip/SubLingo/releases/tag/v0.1.0-alpha.2.
